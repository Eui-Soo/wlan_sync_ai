...
for SNR_loop=1:length(SNR)
    XTest = zeros(length(cfo_range),L_window,1,N_set);
    YTest = zeros(1,1,1,N_set);
    YTest2 = zeros(L_window,1,1,N_set);
    for loop=1:N_set
        %% Data part
        %% data
        %     SNR=-20+30*rand(1);
...
%         end
        ...
        YTest2(label+160,1,1,loop)=1;
        %         YTrain(1,1,2,loop)=cfo;
        %     XTrain2(:,:,1,loop) = corr2;
        %     YTrain2(1,loop)=idx;
    end
    S = sprintf('data_test_set_cfo(%d)_joint_SNR(%d)',cfo_com,SNR(SNR_loop));
    save(S, 'XTest','YTest','YTest2','-v7.3');
end